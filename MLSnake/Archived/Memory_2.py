...
def make_memory():
    phi_size = cfg.game_size * cfg.game_size * cfg.stack_size

    # Replay Memory.
    states_memory = tf.Variable(np.ndarray((cfg.memory_size, phi_size)))
    action_memory = tf.Variable(np.ndarray((cfg.memory_size)))
    reward_memory = tf.Variable(np.ndarray((cfg.memory_size)))
    transitions_memory = tf.Variable(np.ndarray((cfg.memory_size, phi_size)))
...
def load_memory():
    try:
        ...
        states_memory = tf.Variable(
            np.load(cfg.save_path + "/states_memory.npy"))
        action_memory = tf.Variable(
            np.load(cfg.save_path + "/action_memory.npy"))
        reward_memory = tf.Variable(
            np.load(cfg.save_path + "/reward_memory.npy"))
        transitions_memory = tf.Variable(
            np.load(cfg.save_path + "/transitions_memory.npy"))
    except Exception as e:
        print(e)
...
def save_memory(states_memory, action_memory, reward_memory, transitions_memory):
    np.save(cfg.save_path + "/states_memory", states_memory.numpy())
    np.save(cfg.save_path + "/action_memory", action_memory.numpy())
    np.save(cfg.save_path + "/reward_memory", reward_memory.numpy())
    np.save(cfg.save_path + "/transitions_memory", transitions_memory.numpy())
