...
class agent:
    ...
    def edit(self, tensor, index, value):
        data = tensor.numpy()
        data[index] = value
        tensor.assign(data)
    ...
    def update_memory(self, states, action, reward, transitions):
        ...
        update_i = self.train_data["update_index"]

        # Start replacing experiences in the memory from the beginning again.
        if update_i >= cfg.memory_size:
            update_i = 0

        # Insert the experience into replay memory.
        self.edit(self.states_memory, update_i, states)
        self.edit(self.action_memory, update_i, action)
        self.edit(self.reward_memory, update_i, reward)
        self.edit(self.transitions_memory, update_i, transitions)
    ...
    def learn(self):
        ...
        states = tf.gather(self.states_memory, indices=indices)
        actions = tf.gather(self.action_memory, indices=indices)
        rewards = tf.gather(self.reward_memory, indices=indices)
        transitions = tf.gather(self.transitions_memory, indices=indices)
        ...
        target_q_values = tf.dtypes.cast(target_q_values, tf.float64)
        target_q_values = rewards + target_q_values

        masks = tf.one_hot(tf.cast(actions, tf.int64), 5)

        with tf.GradientTape() as tape:
            q_phi = self.q(states)
            q_action = tf.reduce_sum(tf.multiply(q_phi, masks), axis=1)
            loss = tf.keras.losses.mean_squared_error(
                target_q_values, q_action)
        ...
        self.stats["loss"].append(int(loss))
