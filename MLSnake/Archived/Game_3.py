...
'''Changes to v3:
    ...
    at the same time, training each as one
    large batch.'''
...
class game:
    ...
    def __init__(self):
        ...
        self.w.title("Snake Worker #1 [MASTER]")
        ...
        for x in range(1, 4):
            w = tk.Toplevel()
            w.geometry(str(cfg.screen_size + 200) +
                        "x" + str(cfg.screen_size))
            w.resizable(0, 0)
            w.title(f"Snake Worker #{x + 1}")
            c = tk.Canvas(w, bg="#000000",
                                    width=cfg.screen_size + 200, height=cfg.screen_size)
            c.pack()
    ...
    def get_states(self):
        ...
        state = np.zeros((4, cfg.game_size, cfg.game_size), dtype=cfg.memtype)
