'''Changes in v5:
    ...
    DOUBLE DQN:
        AVERAGE VALUES FOR EPSILON
        NEW UPDATE FUNCTION

    Use Memory v4
    Remove instances of self.stack() to accommodate the new CNN in Memory v4
    '''
...
class agent:
    ...
    def __init__(self, game):
        ...
        self.DQN1, self.DQN2 = mem.load_models()
    ...
    def save_all(self, path):
        mem.save_models(path, self.DQN1, self.DQN2)
    ...
    def epsilon_action(self):
        ...
        stack = np.expand_dims(self.phi, axis=0)
        ...
        if random.uniform(0, 1) <= current_epsilon and not cfg.performance_mode:
            prediction = [["n/a", "n/a", "n/a", "n/a", "n/a"]]

            action = cfg.directions[random.randint(0, 4)]
        else:
            # Select the best action, according to the model.
            # TAKE THE MEAN OF THE PREDICTIONS OF BOTH DQNs
            prediction1 = self.DQN1(stack, training=False).numpy()
            prediction2 = self.DQN2(stack, training=False).numpy()
    ...
    def learn(self):
        ...
        # SET WHICH MODEL WILL BE TARGET WHICH WILL BE REAL
        ...
        # MIGHT HAVE TO DO THE CHANGE HERE WHERE TARGET Q IS FOR THE ACTION NOT THE MAX
        with tf.GradientTape() as tape:
            masks = tf.one_hot(actions, 5)

            max_q_phi_next = tf.argmax(q(transitions, training=False), axis=1)
            target_masks = tf.one_hot(max_q_phi_next, 5)
            # q_phi_next = target_q(transitions, training=False)
            q_phi_next = target_q(transitions, training=False)

            # target_q_values = cfg.discount * tf.reduce_max(q_phi_next, axis=1)
            target_q_values = cfg.discount * tf.reduce_sum(tf.multiply(q_phi_next, target_masks), axis=1)
        ...
        # CODE FOR ONE DQN/TARGET Q NETWORK, NOT DOUBLE DQN
    ...
    def add_debug(self):
        ...
        stack = np.expand_dims(test.test_state, axis=0)
        self.stats["testing"].append(
            self.DQN1(stack, training=False).numpy()[0][0])
